...
def build_claim_description(row: pd.Series) -> str:
    """Build a natural-language adjudication prompt from structured claim fields."""
    cashless_str = "cashless" if row["is_cashless"] else "reimbursement"
    blacklist_str = "YES - BLACKLISTED" if row["provider_blacklist_flag"] else "No"
    prior_fraud_str = "YES" if row["previous_fraud_on_policy"] else "No"
    icd_match_str = "Yes" if row["icd_code_matches_procedure"] else "NO - MISMATCH"
    phantom_str = "YES - PHANTOM" if row["is_phantom"] else "No"

    return (
        f"CLAIM ADJUDICATION REQUEST\n"
        f"Claim ID: {row['claim_id']} | Type: {cashless_str} | TPA: {row['tpa']}\n"
        f"Patient: {row['patient_id']} (Age {row['age']}, {row['gender']}, {row['language']}-speaking) | "
        f"State: {row['state']} | Policy: {row['policy_type']} | "
        f"Policy age: {row['days_since_policy_start']} days | "
        f"Pre-existing: {row['pre_existing_conditions']} | "
        f"Identity verified: {row['identity_verified']} ({row['kyc_type']}) | "
        f"Claims (lifetime/12m): {row['number_of_claims_lifetime']}/{row['number_of_claims_last_12m']}\n"
        f"Hospital: {row['hospital_name']} | City: {row['hospital_city']} (Tier {row['city_tier']}) | "
        f"Specialization: {row['specialization']} | "
        f"Phantom: {phantom_str} | Blacklisted: {blacklist_str}\n"
        f"Diagnosis: {row['diagnosis_primary']} ({row['diagnosis_description']}) | "
        f"Admitted: {row['date_of_admission']} | Discharged: {row['date_of_discharge']} | "
        f"Claimed: {row['date_of_claim']} | LOS: {row['length_of_stay_days']} days\n"
        f"Amount requested: INR {int(row['claim_amount_requested_inr']):,} | "
        f"Amount approved: INR {int(row['claim_amount_approved_inr']):,}\n"
        f"Risk signals: "
        f"ICD-procedure match={icd_match_str} | "
        f"Insurers same event={row['num_insurers_same_event']} | "
        f"Prior fraud on policy={prior_fraud_str} | "
        f"Discharge-readmit gap={row['discharge_readmit_gap_days']} days | "
        f"Pharmacy bill ratio={row['pharmacy_bill_ratio']:.2f} | "
        f"Agent: {row['agent_id']} | Ground truth: {row['ground_truth_source']}"
    )
...
def build_adjudication_reasoning(row: pd.Series) -> str:
    ...
    if fraud_type == "legitimate":
        blacklist_note = "Not blacklisted" if not blacklist else "Provider blacklisted - note for audit"
        phantom_note = "No" if not is_phantom else "YES (undetected phantom - escalate)"
        return (
            f"Step 1 (Amount): Requested INR {amount_req:,} for {icd} ({row['diagnosis_description']}). "
            f"Expected range INR {icd_lo:,}-{icd_hi:,}. Ratio {amount_ratio:.2f}x ceiling. PASS.\n"
            f"Step 2 (Policy): Policy age {days_policy} days. Prior fraud: {prior_fraud}. Identity verified: {identity_verified}. PASS.\n"
            f"Step 3 (Provider): {blacklist_note}. Phantom flag: {phantom_note}. PASS.\n"
            f"Step 4 (Clinical): ICD matches procedure. LOS {los} days is clinically consistent. "
            f"Pharmacy ratio {pharma_ratio:.2f} is within norm (<0.45). PASS.\n"
            f"Step 5 (Pattern): Single insurer ({n_insurers}). Readmit gap {readmit_gap} days is normal. "
            f"Claim filed {days_to_file} days after discharge - standard window. PASS.\n"
            f"Decision: APPROVE. All 5 checks passed. Confidence: {conf:.2f}."
        )
